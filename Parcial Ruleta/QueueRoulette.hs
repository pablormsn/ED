...
turn 0 r = r
turn n (R q size)
    | n>0 && n<size = turn (n-1) (R (Q.enqueue p (Q.dequeue q)) size) --(2,3,4,5) --> (3,4,5,2)
    | n>0 && n>size = turn (n-size) (R q size)
    | n> -size = turn (size+n) (R q size)
    | otherwise = turn (n+size) (R q size)
        where
            p = Q.first q
...
listToRoulette :: [a] -> Roulette a
listToRoulette xs = listToRouletteAc xs (Q.empty) 0
    where
        listToRouletteAc [] q size = R q size
        listToRouletteAc (x:xs) q size = listToRouletteAc xs (Q.enqueue x q) (size+1)
...
rouletteToList :: Roulette a -> [a]
rouletteToList (R q 0) = []
rouletteToList r= (sign r:rouletteToList (delete r))
-- ===========================================================
...
-- y luego n a la izquierda produce la misma ruleta. Las ruletas son Arbitrary por lo 
-- que pueden aparecer como argumentos de una propiedad
p1:: Integer -> Roulette Integer -> Bool
p1 n r = turn (-n) (turn n r) == r
