...
greaterSumTree bst = fst (greaterSumTreeAc bst 0)
  where
    greaterSumTreeAc Empty acc = (Empty,acc)
    greaterSumTreeAc (Node x l r) acc = (Node x' l' r',acc3)
      where
        (r',x') = greaterSumTreeAc r acc
        (l',acc3) = greaterSumTreeAc l (x+x')
