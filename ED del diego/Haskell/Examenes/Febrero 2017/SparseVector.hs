...
vector n x = if n < 0 then error "El numero es negativo" else V (2^n) (Unif x) 
...
get x (V _ (Unif y)) = y
get x (V n (Node l r))
  | x >= 0 && x < nl = get x (V nl l)
  | x >= nl && x < n = get (x-nl) (V nl r)
  | otherwise = error "Índice invalido"
  where 
    nl = (div n 2)
...
set pos x (V n t) = V n (setOnTree pos x n t)
  where 
    setOnTree pos x nElms (Unif y)
      | nElms == 1 = (Unif x)
      | pos >= 0 && pos < nElms' = Node (setOnTree pos x nElms' (Unif y)) (Unif y)
      | otherwise = Node (Unif y) (setOnTree (pos-nElms') x nElms' (Unif y))
      where 
        nElms' = div nElms 2

    setOnTree pos x nElms (Node l r)
      | pos >= 0 && pos < nElms' = simplify (setOnTree pos x nElms' l) r
      | pos >= nElms' && pos < nElms = simplify l (setOnTree (pos-nElms') x nElms' r)
      | otherwise = error "Índice incorrecto"
      where 
         nElms' = div nElms 2
...
mapVector f (V n t) = V n (mapTree f t)
  where 
    ...
    mapTree f (Node l r) = simplify (mapTree f l) (mapTree f r)
...
filterVector f (V n t) = filterElmTree f n t
  where 
    filterElmTree f size (Unif x) = if f x then insert x size else []
    filterElmTree f size (Node l r) = (filterElmTree f half l) ++ (filterElmTree f half r)
      where 
        half = div size 2
-------------------------------------------------------------------------------
-- | Exercise i.
depthOf :: Int -> Vector a -> Int
depthOf i (V _ (Unif x)) = 0
depthOf i (V n t) = depthOfTree i n t
  where
    depthOfTree _ _ (Unif y) = 0
    depthOfTree i sz (Node l r)
      | i >= 0 && i < half = 1 + depthOfTree i half l
      | i >= half && i < sz = 1 + depthOfTree (i-half) half r
      | otherwise = error "Índice fuera de los límites" 
      where half = div sz 2
