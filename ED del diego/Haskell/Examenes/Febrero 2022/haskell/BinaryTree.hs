...
greaterSumTree bst = reverseInOrder bst 0
  where
    reverseInOrder Empty suma = Empty
    reverseInOrder (Node a Empty Empty) suma = Node suma Empty Empty 
    reverseInOrder (Node a l r) suma = Node (traverseR r + suma) (reverseInOrder l ((traverseR r) + a + suma)) (reverseInOrder r suma) -- El nodo raíz va a ser igual a la suma acumulada del lado derecho, y el lado izquierdo
                                                                                                        -- va a tener como parámetro suma, la suma acumulada del lado derecho más la raíz
      where
        traverseR Empty = 0
        traverseR (Node x l r) = x + (traverseR l) + (traverseR r)

-- | SOLO PARA ALUMNOS A TIEMPO PARCIAL ---------------------------------------
-- | ONLY FOR PART TIME STUDENTS        ---------------------------------------

mirroredTree :: BinaryTree a -> BinaryTree a
mirroredTree Empty = Empty
mirroredTree (Node x Empty Empty) = Node x Empty Empty
mirroredTree (Node x l r) = Node x l' r' -- Necesitaremos una función que intercambie el árbol izquierdo con el derecho
  where 
    (l',r') = (mirroredTree r, mirroredTree l)
...
traversals2Tree [] [] = Empty
traversals2Tree (x:ps) is = Node x (traversals2Tree lPs lIs) (traversals2Tree rPs rIs) 
  where
    ((lPs,lIs),(rPs,rIs)) = searchSplit x ps is []
