...
aux g dict stack
   | S.isEmpty stack = Just dict  -- Cuando la pila este vacio devolvemos el grafo
   | colored v && c /= (D.valueOf' v dict) = Nothing -- Ha sido visitado
   | not (colored v) = aux g (D.insert v c dict) stack' -- No ha sido visitado
...
--
  where
    v = fst (S.top stack)
    c = snd (S.top stack)
    stack' = pushAll stack [(s,(nextColor c)) | s <- (successors g v)]
