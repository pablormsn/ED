...
// -- EXERCISE 1
  ...
  private static int lengthSeq(LinkedSeq<Integer> s){
    Node<Integer> current = s.first;
    int res = 0;
    while(current != null){
      res++;
  ...
  private static int seq2Num(LinkedSeq<Integer> seq){
    int pos = lengthSeq(seq)-1;
    int res = 0;
    Node<Integer> current = seq.first;
    while(pos >= 0){
      res += current.elem*(Math.pow(10,pos));
      current = current.next;
      pos--;
    }
    return res;
  ...
  public static void addSingleDigit(int d, LinkedSeq<Integer> linkedSeq) {
    // TODO
    // Convertimos nuestra secuencia un número
    linkedSeq.first = fromInt(d + (seq2Num(linkedSeq)),null);
  }
}
