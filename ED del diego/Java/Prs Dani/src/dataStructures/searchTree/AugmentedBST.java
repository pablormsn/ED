...
public class AugmentedBST<T extends Comparable<? super T>> {
    ...
    private T selectRec(Tree<T> node, int i){
        if(node == null){
            return null;
        }else if(i == weight(node.left) + 1){
            return node.key;
        }else if(i < weight(node.left) + 1){
            return selectRec(node.left, i);
        }else{
            return selectRec(node.right, i - (weight(node.left) + 1));
        }
    ...
    public T floor(T k) {
        T res = null;
        for(int i = 0; i < this.size() && this.select(i).compareTo(k) <= 0; i ++){
            res = this.select(i);
        }
        return res;
    }
    ...
    public T ceiling(T k) {
        T res = null;
        for(int i = this.size() - 1; i >= 0 && this.select(i).compareTo(k) >= 0; i --){
            res = this.select(i);
        }
        return res;
    }
    ...
    public int size(T low, T high) {
        int count = 0;
        for(int i = 0; i < this.size() && this.select(i).compareTo(low) >= 0 && this.select(i).compareTo(high) <= 0; i ++){
            count ++;
        }
        return count;
    }
