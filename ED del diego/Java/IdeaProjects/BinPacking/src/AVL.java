...
class Bin {
    ...
    public void addObject(int weight) {
        if(weight>remainingCapacity){
            throw new RuntimeException("Peso superior, el objeto no cabe");
        }
...
public class AVL {
    static private class Node {
        ...
        void setHeight() {
            //Primero hay que ajustar recursivamente las ramas
            int lh =0, rh =0;
            if(left != null){
                left.setHeight();
                lh=left.height;
        ...
        void setMaxRemainingCapacity() {
            ...
            //IMPORTANTE!!!! COMPARAS CON LA CAPACIDAD DEL BIN
        }

        // left-rotates this node. Returns root of resulting rotated tree
        Node rotateLeft() {
            //No tengo que crear un nodo para la raiz porque es el this que llama a la funcion
            Node x = this.right;
            Node r1 = x.left;
    ...
    private void addNewBin(Bin bin) {
        if(root==null){
            root = new Node(); // Inicializamos
            root.bin= bin; // Añadimos cubo
        }else{
            //Recorrer toda la espina derecha para llegar al final
            Node current = root;
            while(current.right != null){
                current= current.right;
            }
            //Mismo proceso que si no hay arbol
            current.right = new Node();
            current.right.bin = bin;

            //Comprobamos si necesita rotacion. COMPROBAR QUE EL HIJO IZQUIERDO NO SEA NULO !!!!
            if(root.right.height - (root.left == null ? 0 : root.left.height)>1){
                root = root.rotateLeft(); //rotateleft devuelve un node por eso lo igualamos a root!!!
            }
        }
        root.setHeight();
