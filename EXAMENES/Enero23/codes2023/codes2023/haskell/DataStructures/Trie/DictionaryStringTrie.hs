...
size :: Trie a -> Int
size Empty = 0
size (Node x dict) = sizeValue x + sum [size (toTrie (D.valueOf v dict)) | v <- D.keys dict ]
...
childOf c (Node x dict) 
  | D.isDefinedAt c dict == False = Empty
  | otherwise = toTrie (D.valueOf c dict)

-- | = Exercise g - search
search :: String -> Trie a -> Maybe a
search _ Empty = Nothing
search [] (Node x dict) = x
search (c:cs) t@(Node x dict)
  | D.isDefinedAt c dict == False = Nothing
  | otherwise = search cs (childOf c t)

-- | = Exercise h - update
update :: Trie a -> Char -> Trie a -> Trie a
update Empty c child = (Node Nothing (D.insert c child D.empty))
update t@(Node x dict) c child 
  |isEmpty (childOf c t) = Node x (D.insert c child dict)
  |otherwise = (Node x (D.updateOrInsert c (\x -> child) (childOf c t) dict))


-- | = Exercise i - insert
insert :: String -> a -> Trie a -> Trie a
insert [] v Empty = (Node (Just v) D.empty)
insert [] v (Node x dict) = (Node (Just v) dict)
insert (c:cs) v t@(Node x dict) = update t c (insert cs v (childOf c t))
