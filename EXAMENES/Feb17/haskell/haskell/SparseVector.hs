...
vector n x
  |n<0 = error "El valor no puede ser negativo"
...
-------------------------------------------------------------------------------
-- | Exercise d.
get :: (Ord a) => Int -> Vector a -> a
get _ (V _ (Unif x)) = x
get i (V n (Node lt rt))
  |i < half = get i (V half lt)
  |otherwise = get (i-half) (V half rt)
    where half = div n 2
...
set i x (V n t) = V n (aux i x n t)
  where
    aux _ x 1 (Unif _) = Unif x
    aux i x n (Unif y) = aux i x n (Node (Unif y) (Unif y))
    aux i x n (Node lt rt)
      |i<half = simplify (aux i x half lt) rt
      |otherwise = simplify lt (aux (i-half) x rt)
        where half = div n 2
