...
public class BiPartite<V> {
	...
	public BiPartite(Graph<V> graph) {
		...
		while (!stack.isEmpty()) {
			...
			stack.pop();
			if(!isDefinedAt(vColor.fst(), dict)){
				dict.insert(vColor.fst(), vColor.snd());
				for(V v : graph.successors(vColor.fst())){
					if(!isDefinedAt(v,dict)){
						if(vColor.snd().equals(Color.Red)){
				...
				if(!(vColor.snd().equals(dict.valueOf(vColor.fst())))){
					isBiColored=false;
					break;
				}
			}
	...
	private boolean isDefinedAt(V key, Dictionary<V, Color> dict){
		for(V v:dict.keys()){
