...
// NO EDITAR EL CÓDIGO DE ARRIBA
    ...
    private static <T> boolean isAuthenticRec(Tree<T> tree){
        if(tree == null){
            return true;
        }else{
            if(!isLeaf(tree) && (tree.left == null || tree.right == null)){
                ...
                return isAuthenticRec(tree.left) && isAuthenticRec(tree.right);
            }
        }
    }
    ...
    private static <T> Tree<T> insertRec(T x, Tree<T> tree){
        return null;
    }
}
