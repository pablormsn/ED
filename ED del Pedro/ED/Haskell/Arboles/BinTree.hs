...
heightB EmptyB = 0
heightB (NodeB x l r) = 1 + max (heightB l) (heightB r)
...
borderB (NodeB x l r) = borderB l ++ borderB r
...
atLevelB l (NodeB _ l' r) = (atLevelB (l-1) l') ++ (atLevelB (l-1) r)
atLevelB _ _ = []
...
pathsToB2 _ EmptyB = []
pathsToB2 x (NodeB y l r)
  | x == y = [[x]] ++ map (y:) (pathsToB x l ++ pathsToB x r)
  | otherwise = map (y:) (pathsToB x l ++ pathsToB x r)
