...
merge Empty m2 = m2
merge m1 Empty = m1
merge h1@(Node x sx lx rx) h2@(Node y sy ly ry)
  | x <= y = Node x (sx+sy) winner (merge lhl rhl)
  | otherwise = Node y (sx+sy) winner' (merge lhl' rhl')
  where
    (lhl,rhl,winner) = ordena3 lhl rhl winner
    (lhl',rhl',winner') = ordena3 lhl' rhl' winner'
-}
...
ordena3 lhl rhl winner
  | (size winner) < (size lhl) && (size winner) < (size rhl) = if (size lhl) >= (size rhl) then (lhl,winner,rhl) else (rhl,lhl,winner)
  | otherwise = (winner,lhl,rhl)
...
ordena3 :: Heap a -> Heap a -> Heap a -> (Heap a, Heap a, Heap a)
ordena3 lhl rhl winner
  | (size winner) < (size lhl) && (size winner) < (size rhl) = if (size lhl) >= (size rhl) then (winner,rhl,lhl) else (lhl,winner,rhl)
  | otherwise = (lhl,rhl,winner)
-}
