...
turn 0 r = r
turn n (R q size)
    | n > 0 && n < size = turn (n-1) (R (Q.enqueue f (Q.dequeue q)) size)
...
delete :: Roulette a ->  Roulette a
delete r@(R q size) = if size == 0 then empty else (R (Q.dequeue q) (size-1)) 
...
mapRoulette :: (a -> b) -> Roulette a -> Roulette b
mapRoulette f r = listToRoulette (map f (rouletteToList r))
