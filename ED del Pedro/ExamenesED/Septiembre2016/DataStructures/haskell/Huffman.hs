...
weights l = aux l D.empty
  where
    aux [] d = d
    aux (h : hs) d
      | D.isDefinedAt h d = aux hs (D.insert h (fromJust (D.valueOf h d) + 1) d)
      | otherwise = aux hs (D.insert h 1 d)
...
huffmanLeaves s = aux (nub s) (weights s) (PQ.empty)
  where
    aux [] w q = q
    aux (h : hs) w q = aux hs w (PQ.enqueue (WLeaf h (fromJust (D.valueOf h w))) q)
...
huffmanTree s = if length (nub s) < 2 then error "the string must have at least two different symbols" else aux (PQ.first (huffmanLeaves s)) (PQ.dequeue (huffmanLeaves s)) 
  where
    aux f pq
      | PQ.isEmpty pq = f
      | otherwise = aux (merge f (PQ.first pq)) (PQ.dequeue pq)
...
joinDics d1 d2 = aux (D.keysValues d1) d2
  where
    aux [] d2 = d2
    aux (h : hs) d2 = aux hs (D.insert k v d2)
      where (k, v) = h
{-
...
prefixWith x d = aux (zip (D.keys d) (map (x:) (D.values d))) d
  where
    aux [] d = d
    aux (h : hs) d = aux hs (D.insert k v d)
      where (k, v) = h
...
huffmanCode ht = aux ht D.empty D.empty
  where
    aux (WNode (WLeaf x xw) (WLeaf y yw) tw) d1 d2 = joinDics (D.insert x [0] d1) (D.insert y [1] d2)
    aux (WNode lt (WLeaf y yw) tw) d1 d2 = joinDics (prefixWith 0 (aux lt d1 d2)) (D.insert y [1] d2)
