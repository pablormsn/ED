...
-- "abcdr"
keys :: Ord a => Bag a -> [a]
keys b = foldBag (\ x _ sig -> x:sig ) [] b
...
bag2List b = foldBag (\ x num sig -> (x, num):sig) [] b
...
cardinal b = foldBag (\  _ num sig -> num + sig) 0 b
...
contains b bag = foldBag (\  a _ sig -> if a == b then True else sig) False bag
...
maxOcurrences bag = foldBag (\ _ num sig -> max num sig) 0 bag 
