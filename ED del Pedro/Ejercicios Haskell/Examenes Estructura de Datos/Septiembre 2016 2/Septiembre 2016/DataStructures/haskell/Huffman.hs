...
frecuencia :: Ord a => a -> [a] -> Int
frecuencia x [] = 0
frecuencia x (y:ys) = if x == y then 1 + frecuencia x ys else frecuencia x ys

weights :: Ord a => [a] -> D.Dictionary a Int
weights [] = D.empty
weights ws = aux (nub ws) D.empty
  where
    aux [] dicNuevo = dicNuevo
    aux (s:res) dicNuevo = D.insert s (frecuencia s ws) (aux res dicNuevo) 
...
huffmanLeaves s = foldr (\ c -> PQ.enqueue (WLeaf c (aux (D.valueOf c (weights s))))) PQ.empty (nub s)
  where
    aux (Just n) = n
...
huffmanTree s = if length (nub s) < 2 then error "the string must have at least two different symbols" else buildTree (huffmanLeaves s)
  where
    buildTree colaP = if not(PQ.isEmpty (PQ.dequeue colaP)) then buildTree (PQ.enqueue (merge (PQ.first colaP) (PQ.first (PQ.dequeue colaP))) (PQ.dequeue (PQ.dequeue colaP))) else (PQ.first colaP)
...
joinDics d1 d2 = union (D.keysValues d1) d2
  where
    union [] d = d
    union ((k,v):resCola) d = D.insert k v (union resCola d)
...
prefixWith x d = modifyValues x (D.empty) (D.keysValues d)
  where 
    modifyValues x dicNuevo kv = foldr (\ (k,v) -> D.insert k (x:v)) dicNuevo kv
...
huffmanCode t = aux t D.empty
  where
    aux (WNode (WLeaf x w) (WLeaf x' w') n) dicNuevo = D.insert x [0] (D.insert x' [1] dicNuevo)
    aux (WNode l (WLeaf x' w') n) dicNuevo = D.insert x' [1] (prefixWith 0 (aux l dicNuevo))
    aux (WNode (WLeaf x w) r n) dicNuevo =  D.insert x [0] ( prefixWith 1 (aux r dicNuevo))
    aux (WNode l r n) dicNuevo = joinDics (prefixWith 0 (aux l dicNuevo)) (prefixWith 1 (aux r dicNuevo))
{-
