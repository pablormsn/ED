...
vector n x = if n<0 then error "Tamaño no válido" else V n (Unif x)
...
get:: Int-> Vector a -> a 
get x (V n (Unif v)) = v
get x (V n (Node lt rt))
   | x < mitad = get x (V mitad lt)
   |x >= mitad = get (x-mitad) (V mitad rt)
   where
      mitad = div n 2 
...
set pos x (V n t) = (V n (setEnTree pos x n t))
  where 
  setEnTree pos x 1 (Unif a) = (Unif x)
  setEnTree pos x tam (Unif a) = setEnTree pos x tam (Node(Unif a)(Unif a))
  setEnTree pos x tam (Node lt rt) 
   |pos < mitad = (simplify (setEnTree pos x  mitad lt) rt)
   | pos >= mitad = (simplify lt (setEnTree (pos-mitad) x  mitad rt))
    where 
      mitad = div tam 2
