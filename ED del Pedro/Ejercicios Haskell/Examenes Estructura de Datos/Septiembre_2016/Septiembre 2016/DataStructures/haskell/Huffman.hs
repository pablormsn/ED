...
huffmanTree s = if length (nub s) < 2 then error "the string must have at least two different symbols" else buildTree (queue2List (huffmanLeaves s))
  where
    buildTree (q:qs) = undefined
