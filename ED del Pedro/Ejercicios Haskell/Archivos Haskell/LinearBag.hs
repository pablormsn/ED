...
insert x (Node y oy b)
   | x < y = Node x 1 (Node y oy b)
   | x == y = Node x (oy+1) b
   | otherwise = Node y oy (insert x b)
...
delete x (Node y oy b)
   | x < y = Node y oy b
   | x == y = if oy == 1 then b else Node y (oy-1) b
   | otherwise = Node y oy (delete x b)
...
intersection (Node x ox b1) (Node y oy b2)
   | x < y = intersection b1 (Node y oy b2)
   | x == y = Node x (min ox oy) (intersection b1 b2)
   | otherwise = intersection (Node x ox b1) b2
-- |
-- >>> difference b1 b2
-- LinearBag { 'e' 'h' 'k' 'l' 'l' 's' }
-- >>> difference b2 b1
