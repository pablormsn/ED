...
aux g dict stack
   ...
   | not (D.isDefinedAt (fst (S.top stack)) dict) = aux g (D.insert (fst (S.top stack)) (snd (S.top stack)) dict) (pushAll (S.pop stack) sucesores)
   | (snd (S.top stack)) /= actualTupleColor = Nothing
   | otherwise = aux g dict (S.pop stack)
      where 
          Just actualTupleColor = (D.valueOf (fst (S.top stack)) dict)
          color = nextColor (snd (S.top stack))
          sucesores = foldr (\v ac -> (v, color):ac) [] (successors g (fst (S.top stack))) 
-- 
